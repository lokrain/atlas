// Runtime/Descriptors/AtlasDescriptor.cs

using System.Runtime.CompilerServices;
using System.Runtime.InteropServices;

namespace Lokrain.Atlas.Descriptors
{
    /// <summary>
    /// Describes one Field contract row in an Atlas descriptor table.
    /// </summary>
    /// <remarks>
    /// <para>
    /// A descriptor is schema metadata. It defines stable identity, storage format, ownership,
    /// lifetime, length shape, Field flags, hash participation, and diagnostic name for one
    /// Atlas Field.
    /// </para>
    ///
    /// <para>
    /// Descriptors do not own memory. Runtime storage is allocated from a validated descriptor
    /// table, then jobs receive already-resolved native containers.
    /// </para>
    ///
    /// <para>
    /// The descriptor slot is table-local. An unslotted descriptor may be created from a Field
    /// declaration and later assigned a canonical slot by <see cref="AtlasDescriptorTable"/>.
    /// </para>
    /// </remarks>
    [StructLayout(LayoutKind.Sequential)]
    public readonly struct AtlasDescriptor :
        IEquatable<AtlasDescriptor>
    {
        /// <summary>
        /// Reserved invalid descriptor.
        /// </summary>
        public static readonly AtlasDescriptor Empty = default;

        /// <summary>
        /// Durable, versioned Field identity.
        /// </summary>
        public readonly StableDataId StableId;

        /// <summary>
        /// Canonical table-local Field slot.
        /// </summary>
        /// <remarks>
        /// The slot may be invalid before a descriptor is placed into a descriptor table.
        /// </remarks>
        public readonly AtlasFieldSlot Slot;

        /// <summary>
        /// Physical unmanaged storage format required by the Field.
        /// </summary>
        public readonly StorageFormat StorageFormat;

        /// <summary>
        /// Allocation and disposal ownership policy for the Field storage.
        /// </summary>
        public readonly OwnershipPolicy Ownership;

        /// <summary>
        /// Validity interval policy for the Field storage.
        /// </summary>
        public readonly LifetimePolicy Lifetime;

        /// <summary>
        /// Rule used to resolve Field length or capacity before scheduling.
        /// </summary>
        public readonly LengthShape LengthShape;

        /// <summary>
        /// Durable behavior flags for the Field.
        /// </summary>
        public readonly AtlasFieldFlags Flags;

        /// <summary>
        /// Hash participation policy for descriptor, shape, compatibility, and content hashes.
        /// </summary>
        public readonly HashParticipation HashParticipation;

        /// <summary>
        /// Stable diagnostic name used by validation, tooling, exceptions, and tests.
        /// </summary>
        /// <remarks>
        /// Debug names are not durable identity. Use <see cref="StableId"/> for identity.
        /// </remarks>
        public readonly FixedString64Bytes DebugName;

        /// <summary>
        /// Creates an Atlas descriptor from explicit schema fields.
        /// </summary>
        /// <param name="stableId">Durable, versioned Field identity.</param>
        /// <param name="slot">Table-local Field slot. May be invalid before table assignment.</param>
        /// <param name="storageFormat">Physical unmanaged storage format.</param>
        /// <param name="ownership">Allocation and disposal ownership policy.</param>
        /// <param name="lifetime">Storage validity interval policy.</param>
        /// <param name="lengthShape">Rule used to resolve length or capacity.</param>
        /// <param name="flags">Durable Field behavior flags.</param>
        /// <param name="hashParticipation">Hash participation policy.</param>
        /// <param name="debugName">Stable diagnostic name.</param>
        public AtlasDescriptor(
            StableDataId stableId,
            AtlasFieldSlot slot,
            StorageFormat storageFormat,
            OwnershipPolicy ownership,
            LifetimePolicy lifetime,
            LengthShape lengthShape,
            AtlasFieldFlags flags,
            HashParticipation hashParticipation,
            FixedString64Bytes debugName)
        {
            StableId = stableId;
            Slot = slot;
            StorageFormat = storageFormat;
            Ownership = ownership;
            Lifetime = lifetime;
            LengthShape = lengthShape;
            Flags = flags;
            HashParticipation = hashParticipation;
            DebugName = debugName;
        }

        /// <summary>
        /// Gets whether this descriptor has a valid Field identity and storage contract.
        /// </summary>
        /// <remarks>
        /// This property does not require a valid slot. Unslotted descriptors are valid before
        /// descriptor-table construction assigns canonical slots.
        /// </remarks>
        public bool IsValid
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get => StableId.IsValid &&
                   StorageFormat.IsValid &&
                   Ownership != OwnershipPolicy.None &&
                   Lifetime != LifetimePolicy.None &&
                   LengthShape.IsValid &&
                   !DebugName.IsEmpty;
        }

        /// <summary>
        /// Gets whether this descriptor has been assigned a canonical table slot.
        /// </summary>
        public bool IsSlotted
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get => Slot.IsValid;
        }

        /// <summary>
        /// Gets whether this descriptor is valid and has a canonical table slot.
        /// </summary>
        public bool IsTableReady
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get => IsValid && IsSlotted;
        }

        /// <summary>
        /// Creates an unslotted descriptor from a typed Atlas Field declaration.
        /// </summary>
        /// <typeparam name="TField">Field declaration type.</typeparam>
        /// <typeparam name="TElement">Unmanaged element type stored by the Field.</typeparam>
        /// <returns>
        /// An unslotted descriptor whose metadata is read from <c>default(TField)</c>.
        /// </returns>
        /// <exception cref="AtlasException">
        /// Thrown when the Field declaration is invalid.
        /// </exception>
        public static AtlasDescriptor Of<TField, TElement>()
            where TField : struct, IAtlasField<TElement>
            where TElement : unmanaged
        {
            AtlasField.ValidateDeclarationOrThrow<TField, TElement>();

            var Field = default(TField);

            return new AtlasDescriptor(
                stableId: Field.StableId,
                slot: AtlasFieldSlot.Invalid,
                storageFormat: StorageFormat.Create<TElement>(Field.StorageKind),
                ownership: Field.Ownership,
                lifetime: Field.Lifetime,
                lengthShape: Field.LengthShape,
                flags: Field.Flags,
                hashParticipation: Field.HashParticipation,
                debugName: Field.DebugName);
        }

        /// <summary>
        /// Creates a descriptor copy with a canonical table slot.
        /// </summary>
        /// <param name="slot">Canonical table-local Field slot.</param>
        /// <returns>A copy of this descriptor with the supplied slot.</returns>
        /// <exception cref="InvalidOperationException">
        /// Thrown when <paramref name="slot"/> is invalid.
        /// </exception>
        public AtlasDescriptor WithSlot(AtlasFieldSlot slot)
        {
            slot.ThrowIfInvalid();

            return new AtlasDescriptor(
                StableId,
                slot,
                StorageFormat,
                Ownership,
                Lifetime,
                LengthShape,
                Flags,
                HashParticipation,
                DebugName);
        }

        /// <summary>
        /// Creates a descriptor copy with a canonical table slot from a zero-based index.
        /// </summary>
        /// <param name="slotIndex">Zero-based descriptor-table slot.</param>
        /// <returns>A copy of this descriptor with the supplied slot.</returns>
        /// <exception cref="ArgumentOutOfRangeException">
        /// Thrown when <paramref name="slotIndex"/> is outside the valid Atlas slot range.
        /// </exception>
        public AtlasDescriptor WithSlot(int slotIndex)
        {
            return WithSlot(AtlasFieldSlot.FromIndex(slotIndex));
        }

        /// <summary>
        /// Throws when this descriptor is invalid for descriptor-table construction.
        /// </summary>
        /// <param name="parameterName">Optional parameter name used by the thrown exception.</param>
        /// <exception cref="ArgumentException">
        /// Thrown when the descriptor is missing required schema metadata.
        /// </exception>
        /// <remarks>
        /// This validation does not require a slot. Descriptor tables assign canonical slots after
        /// receiving ordered descriptors.
        /// </remarks>
        public void ValidateOrThrow(string parameterName = null)
        {
            var name = parameterName ?? nameof(AtlasDescriptor);

            if (!StableId.IsValid)
            {
                throw new ArgumentException(
                    $"Atlas descriptor '{DebugName}' has an invalid stable id.",
                    name);
            }

            StorageFormat.ValidateOrThrow(name);
            LengthShape.ValidateOrThrow(name);

            if (Ownership == OwnershipPolicy.None)
            {
                throw new ArgumentException(
                    $"Atlas descriptor '{DebugName}' has no ownership policy.",
                    name);
            }

            if (Lifetime == LifetimePolicy.None)
            {
                throw new ArgumentException(
                    $"Atlas descriptor '{DebugName}' has no lifetime policy.",
                    name);
            }

            if (DebugName.IsEmpty)
            {
                throw new ArgumentException(
                    "Atlas descriptor has an empty debug name.",
                    name);
            }

            ValidatePolicyCombinationOrThrow(name);
        }

        /// <summary>
        /// Throws when this descriptor is invalid for runtime table usage.
        /// </summary>
        /// <param name="parameterName">Optional parameter name used by the thrown exception.</param>
        /// <exception cref="ArgumentException">
        /// Thrown when the descriptor is invalid or has no canonical slot.
        /// </exception>
        public void ValidateTableReadyOrThrow(string parameterName = null)
        {
            ValidateOrThrow(parameterName);

            if (Slot.IsValid)
            {
                return;
            }

            throw new ArgumentException(
                $"Atlas descriptor '{DebugName}' has no assigned table slot.",
                parameterName ?? nameof(AtlasDescriptor));
        }

        /// <summary>
        /// Determines whether this descriptor is equal to another descriptor.
        /// </summary>
        /// <param name="other">The descriptor to compare with this descriptor.</param>
        /// <returns><c>true</c> when all descriptor fields match; otherwise, <c>false</c>.</returns>
        public bool Equals(AtlasDescriptor other)
        {
            return StableId == other.StableId &&
                   Slot == other.Slot &&
                   StorageFormat == other.StorageFormat &&
                   Ownership == other.Ownership &&
                   Lifetime == other.Lifetime &&
                   LengthShape == other.LengthShape &&
                   Flags == other.Flags &&
                   HashParticipation == other.HashParticipation &&
                   DebugName.Equals(other.DebugName);
        }

        /// <summary>
        /// Determines whether this descriptor is equal to an object instance.
        /// </summary>
        /// <param name="obj">The object to compare with this descriptor.</param>
        /// <returns>
        /// <c>true</c> when <paramref name="obj"/> is an <see cref="AtlasDescriptor"/> with
        /// matching fields.
        /// </returns>
        public override bool Equals(object obj)
        {
            return obj is AtlasDescriptor other && Equals(other);
        }

        /// <summary>
        /// Returns a hash code suitable for managed hash containers.
        /// </summary>
        /// <returns>A managed hash code for this descriptor.</returns>
        public override int GetHashCode()
        {
            unchecked
            {
                var hash = StableId.GetHashCode();
                hash = (hash * 397) ^ Slot.GetHashCode();
                hash = (hash * 397) ^ StorageFormat.GetHashCode();
                hash = (hash * 397) ^ (int)Ownership;
                hash = (hash * 397) ^ (int)Lifetime;
                hash = (hash * 397) ^ LengthShape.GetHashCode();
                hash = (hash * 397) ^ (int)Flags;
                hash = (hash * 397) ^ (int)HashParticipation;
                hash = (hash * 397) ^ DebugName.GetHashCode();
                return hash;
            }
        }

        /// <summary>
        /// Returns a diagnostic representation of this descriptor.
        /// </summary>
        /// <returns>A string containing debug name, stable identity, slot, and storage format.</returns>
        public override string ToString()
        {
            return $"{DebugName} [{StableId}] Slot={Slot} Storage={StorageFormat}";
        }

        /// <summary>
        /// Determines whether two descriptors are equal.
        /// </summary>
        /// <param name="left">The first descriptor.</param>
        /// <param name="right">The second descriptor.</param>
        /// <returns><c>true</c> when all descriptor fields match; otherwise, <c>false</c>.</returns>
        public static bool operator ==(AtlasDescriptor left, AtlasDescriptor right)
        {
            return left.Equals(right);
        }

        /// <summary>
        /// Determines whether two descriptors are not equal.
        /// </summary>
        /// <param name="left">The first descriptor.</param>
        /// <param name="right">The second descriptor.</param>
        /// <returns><c>true</c> when any descriptor field differs; otherwise, <c>false</c>.</returns>
        public static bool operator !=(AtlasDescriptor left, AtlasDescriptor right)
        {
            return !left.Equals(right);
        }

        private void ValidatePolicyCombinationOrThrow(string parameterName)
        {
            if (Flags.HasAll(AtlasFieldFlags.ClearOnAcquire | AtlasFieldFlags.PreserveContent))
            {
                throw new ArgumentException(
                    $"Atlas descriptor '{DebugName}' declares both " +
                    $"{nameof(AtlasFieldFlags.ClearOnAcquire)} and {nameof(AtlasFieldFlags.PreserveContent)}.",
                    parameterName);
            }

            if (Flags.HasAny(AtlasFieldFlags.AllowsUninitializedMemory) &&
                Flags.HasNone(AtlasFieldFlags.DiscardBeforeWrite))
            {
                throw new ArgumentException(
                    $"Atlas descriptor '{DebugName}' declares " +
                    $"{nameof(AtlasFieldFlags.AllowsUninitializedMemory)} without " +
                    $"{nameof(AtlasFieldFlags.DiscardBeforeWrite)}.",
                    parameterName);
            }

            if (Flags.HasAny(AtlasFieldFlags.Resizable) &&
                !StorageFormat.SupportsResize)
            {
                throw new ArgumentException(
                    $"Atlas descriptor '{DebugName}' is marked resizable, but storage kind " +
                    $"'{StorageFormat.Kind}' does not support resizing.",
                    parameterName);
            }

            if (StorageFormat.Kind == StorageKind.External &&
                Ownership != OwnershipPolicy.ExternalOwned &&
                Ownership != OwnershipPolicy.Borrowed &&
                Ownership != OwnershipPolicy.Imported &&
                Ownership != OwnershipPolicy.Adopted)
            {
                throw new ArgumentException(
                    $"Atlas descriptor '{DebugName}' uses external storage with incompatible ownership policy '{Ownership}'.",
                    parameterName);
            }

            if (Lifetime == LifetimePolicy.External &&
                Ownership != OwnershipPolicy.ExternalOwned &&
                Ownership != OwnershipPolicy.Borrowed &&
                Ownership != OwnershipPolicy.Imported)
            {
                throw new ArgumentException(
                    $"Atlas descriptor '{DebugName}' uses external lifetime with incompatible ownership policy '{Ownership}'.",
                    parameterName);
            }

            if (StorageFormat.Kind == StorageKind.Scalar &&
                !LengthShape.IsScalar)
            {
                throw new ArgumentException(
                    $"Atlas descriptor '{DebugName}' uses scalar storage but does not declare scalar length shape.",
                    parameterName);
            }
        }
    }
}